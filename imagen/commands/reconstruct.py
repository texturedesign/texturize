...
def main(args):
    ...
    pb = patches.PatchBuilder(
        patch_size=3, weights=[0.1, 0.1, 0.1, 0.1, 4.0, 0.1, 0.1, 0.1, 0.1]
    )
    ...
    for p in range(args.passes):
        ...
        content_ptch = content_ptch * 0.5 + 0.5 * patchmatch.torch_gather_2d(
            style_ptch, pm.indices
        )
    ...
    images.save_to_file(
        recons_tensor, os.path.splitext(args.content)[0] + "_recons.png", mode="YCbCr"
    )
