...
class PatchBuilder:
    ...
    def extract(self, array):
        padded = torch_pad_replicate(
            array, (abs(self.min), self.max, abs(self.min), self.max)
        )
        h, w = (
    ...
    def reconstruct(self, patches):
        ...
        output = patches.new_zeros(
            (patches.shape[0], patches.shape[1] // layer_count) + (oh, ow)
        )
        weights = torch.zeros(
            (patches.shape[0], 1, oh, ow), dtype=torch.float, device=patches.device
        )

        ph, pw = patches.shape[2:]
        for i, (y, x) in enumerate(itertools.product(self.coords, repeat=2)):
            output[:, :, y : ph + y, x : pw + x] += (
                layers[:, i, :, :, :] * self.weights[i]
            )
            weights[:, 0, y : ph + y, x : pw + x] += self.weights[i]

        weights[weights == 0.0] = 1.0
        return (output / weights)[
            :, :, abs(self.min) : oh - self.max, abs(self.min) : ow - self.max
        ]
