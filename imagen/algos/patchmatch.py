...
class PatchMatcher:
    ...
    def create_indices_random(self):
        ...
        torch.randint(
            low=0,
            high=self.style.shape[2],
            size=self.content.shape[-2:],
            out=indices[:, 0, :, :],
        )
        torch.randint(
            low=0,
            high=self.style.shape[3],
            size=self.content.shape[-2:],
            out=indices[:, 1, :, :],
        )
        return indices
    ...
    def create_indices_linear(self):
        ...
        indices[:, 0, :, :] = (
            ...
            .mul(self.style.shape[2] / h)
            .view((b, -1, 1))
            .long()
        )
        indices[:, 1, :, :] = (
            torch.arange(w, dtype=torch.float)
            .mul(self.style.shape[3] / w)
            .view((b, 1, -1))
            .long()
        )
        return indices
    ...
    def improve_patches(self, candidate_indices):
        ...
        candidate_scores = torch.sum(
            self.content * candidate_repro, dim=1, keepdim=True
        )
    ...
    def search_patches_propagate(self, steps=[1, 2, 4, 8]):
        ...
        for y, x in cells:
            ...
            lookup[:, 0, :, :] = torch.arange(
                0, lookup.shape[2], dtype=torch.long
            ).view((1, -1, 1))
            ...
            indices = torch_pad_replicate(
                self.indices.float(), (padding, padding, padding, padding)
            ).long()
            indices[:, 0, :, :] -= y
            indices[:, 1, :, :] -= x
